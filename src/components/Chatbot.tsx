...
import { botReply, suggestedQuestions, welcomeMessage, type BotReply } from '@/data/chatbot';
...
export function Chatbot({ onProductClick }: ChatbotProps) {
  ...
  useEffect(() => {
    ...
    if (stored.length) {
      ...
      const intro: ChatMessage = { role: 'bot', text: welcomeMessage, time: now() };
      setMessages([intro]);
        ...
        <div className="chat-panel" role="dialog" aria-label="Ayurique customer care chat">
          <div className="chat-header">
            <div className="chat-header-info">
              ...
              <span className="chat-sub">
                <span className="chat-online-dot" /> Customer Care
              </span>
            </div>
            <div className="chat-header-actions">
              <button onClick={clearChat} aria-label="Clear conversation" className="chat-clear-btn">
                <Trash2 size={16} />
              </button>
              <button onClick={() => setOpen(false)} aria-label="Close chat" className="chat-close-btn">
                <X size={20} />
              </button>
            </div>
          </div>
          ...
          <form className="chat-input-row" onSubmit={(e) => { e.preventDefault(); send(input); }}>
            <input
              ...
              placeholder="Ask about products, offers, ordering…"
